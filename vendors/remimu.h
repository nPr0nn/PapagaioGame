...
#define REMIMU_ITERATION_LIMIT                                                 \
  0 // Set to non-zero to enable an interation limit
#endif
...
/************
    ...
    Compatible with C99 and C++11 and later standards. Uses backtracking and
relatively standard regex syntax.

    #include "remimu.h"

FUNCTIONS

    // Returns 0 on success, or -1 on invalid or unsupported regex, or -2 on not
enough tokens given to parse regex. int regex_parse( const char * pattern, //
Regex pattern to parse. RegexToken * tokens,        // Output buffer of
token_count regex tokens. int16_t * token_count,      // Maximum allowed number
of tokens to write int32_t flags               // Optional bitflags.
    )

    // Returns match length, or -1 on no match, or -2 on out of memory, or -3 if
the regex is invalid. int64_t regex_match( const RegexToken * tokens,  // Parsed
regex to match against text. const char * text,          // Text to match
against tokens. size_t start_i,             // index value to match at. uint16_t
cap_slots,         // Number of allowed capture info output slots. int64_t *
cap_pos,          // Capture position info output buffer. int64_t * cap_span //
Capture length info output buffer.
    )

    void print_regex_tokens(
        RegexToken * tokens     // Regex tokens to spew to stdout, for
debugging.
    )

PERFORMANCE

    On simple cases, Remimu's match speed is similar to PCRE2. Regex
parsing/compilation is also much faster (around 4x to 10x), so single-shot
regexes are often faster than PCRE2.

    HOWEVER: Remimu is a pure backtracking engine, and has `O(2^x)` complexity
on regexes with catastrophic backtracking. It can be much, much, MUCH slower
than PCRE2. Beware!

    Remimu uses length-checked fixed memory buffers with no recursion, so memory
usage is statically known.
...
FEATURES
    ...
    - - \b, \B word boundary and non-word-boundary anchors (not fully supported
in zero-size quantified groups, but even then, usually supported)
    - - Escaped literal characters: {}[]-()|^$*+?:./\
    - - - Escapes work in character classes, except for 'b'
    - Character classes, including disjoint ranges, proper handling of bare [
and trailing -, etc
    - - Dot (.) matches all characters, including newlines, unless
REMIMU_FLAG_DOT_NO_NEWLINES is passed as a flag to regex_parse
...
requires two dots (and not using REMIMU_FLAG_DOT_NO_NEWLINES)
    ...
    - - NOTE: Capture groups for and inside of possessive groups return no
capture information.
    - Atomic groups e.g. (?>(asdf))
    - - NOTE: Capture groups inside of atomic groups return no capture
information.

NOT SUPPORTED

    - Strings with non-terminal null characters
    - Unicode character classes (matching single utf-8 characters works
regardless)
    ...
    - Capture of or inside of possessive-quantified groups (still take up a
capture slot, but no data is returned)
...
USAGE
    ...
    int64_t matchlen = regex_match(tokens, "aaaaaabbbabaqa", 0, 5, cap_pos,
cap_span); printf("Match length: %zd\n", matchlen); for (int i = 0; i < 5; i++)
...
REMIMU_CONST_VISIBILITY uint8_t REMIMU_MODE_INVERTED =
    128; // temporary; gets cleared later
...
typedef struct _RegexToken {
  ...
  uint16_t mask[16];   // for groups: mask 0 stores group-with-quantifier number
                       // (quantifiers are +, *, ?, {n}, {n,}, or {n,m})
  int16_t pair_offset; // from ( or ), offset in token list to matching paren.
                       // TODO: move into mask maybe
} RegexToken;
...
/// -2: Provided buffer not long enough. Give up, or reallocate with more length
/// and retry. Returns 0 on success. On call, token_count pointer must point to
/// the number of tokens that can be written to the tokens buffer. On successful
/// return, the number of actually used tokens is written to token_count. Sets
/// token_count to zero if a regex is not created but no error happened (e.g.
/// empty pattern). Flags: Not yet used. SAFETY: Pattern must be
/// null-terminated. SAFETY: tokens buffer must have at least the input
/// token_count number of RegexToken objects. They are allowed to be
/// uninitialized.
REMIMU_FUNC_VISIBILITY int regex_parse(const char *pattern, RegexToken *tokens,
                                       int16_t *token_count, int32_t flags) {
...
#define _REGEX_PUSH_TOKEN()                                                    \
  do {                                                                         \
    if (k == 0 || tokens[k - 1].kind != token.kind ||                          \
...
#define _REGEX_SET_MASK_ALL()                                                  \
  ...
  // (this allows the matcher to not need to have a special root-level
  // alternation operator case)
  ...
  for (uint64_t i = 0; i < pattern_len; i++) {
    ...
    if (state == STATE_NORMAL) {
      if (esc_state == 1) {
        ...
        if (c == '\\') {
          ...
          for (ptrdiff_t l = k - 1; l >= 0; l--) {
            if (tokens[l].kind == REMIMU_KIND_NCOPEN ||
                tokens[l].kind == REMIMU_KIND_OPEN) {
              if (balance == 0) {
                found = l;
                break;
              } else
    ...
    } else if (state == STATE_CC_INIT || state == STATE_CC_NORMAL ||
      ...
      if (esc_state == 1) {
        ...
        if (c == 'n')
          ...
          printf(
              "unknown/unsupported escape sequence in character class (\\%c)\n",
              c);
          return -1; // unknown/unsupported escape sequence
        }
      }
      if (state == STATE_CC_INIT) {
        ...
        if (c == ']' && esc_c == 0) {
          ...
          if (char_class_mem == -1) {
            ...
            return -1; // probably tried to use a character class shorthand as
                       // part of a range
          }
  ...
  *token_count = k;
  ...
  // also take the opportunity to smuggle "quantified group index" into the mask
  // field for the )
  uint64_t n = 0;
      ...
      for (ptrdiff_t l = k2 + 1; l < k; l++) {
        if (tokens[l].kind == REMIMU_KIND_OR && balance == 0) {
          ...
          if (balance == 0) {
            ...
            balance -= 1;
        } else if (tokens[l].kind == REMIMU_KIND_NCOPEN ||
                   tokens[l].kind == REMIMU_KIND_OPEN)
          balance += 1;
      }
      if (found == -1) {
...
typedef struct _RegexMatcherState {
  uint32_t k;
  uint32_t
      group_state; // quantified group temp state (e.g. number of repetitions)
  uint32_t
      prev; // for )s, stack index of corresponding previous quantified state
...
// NOTE: undef'd later
#define _REGEX_CHECK_MASK(K, byte)                                             \
...
// The first cap_slots capture positions and spans (lengths) will be written to
// cap_pos and cap_span. If zero, will not be written to. SAFETY: The text
// variable must be null-terminated, and start_i must be the index of a
// character within the string or its null terminator. SAFETY: Tokens array must
// be terminated by a REMIMU_KIND_END token (done by default by regex_parse).
// SAFETY: Partial capture data may be written even if the match fails.
REMIMU_FUNC_VISIBILITY int64_t regex_match(const RegexToken *tokens,
                                           const char *text, size_t start_i,
                                           uint16_t cap_slots, int64_t *cap_pos,
                                           int64_t *cap_span) {
  (void)text;
...
#define IF_VERBOSE(X)                                                          \
  ...
  uint32_t
      q_group_stack[aux_stats_size]; // location of most recent corresponding )
                                     // on stack. 0 means nowhere
...
#define _REWIND_DO_SAVE_RAW(K, ISDUMMY)                                        \
  do {                                                                         \
    ...
    _P_TEXT_HIGHLIGHTED();                                                     \
    IF_VERBOSE(printf("-- saving rewind state k %u i %zd rmin %zu rmax %zd "   \
                      "(line %d) (depth %d prev %d)\n",                        \
...
#define _REWIND_OR_ABORT()                                                     \
  do {                                                                         \
    ...
    IF_VERBOSE(                                                                \
        printf(                                                                \
            "-- rewound to k %u i %zd rmin %zu rmax %zd (kind %d prev %d)\n",  \
            k, i, range_min, range_max, tokens[k].kind,                        \
            rewind_stack[stack_n].prev);)                                      \
    k -= 1;                                                                    \
  } while (0)
  ...
  w_mask[6] = 0xFFFE;
  w_mask[7] = 0x07FF;
#define _REGEX_CHECK_IS_W(byte)                                                \
  (!!(w_mask[((uint8_t)byte) >> 4] & (1 << ((uint8_t)byte & 0xF))))
  ...
  for (k = 0; k < tokens_len; k++) {
    ...
    IF_VERBOSE(
        printf("k: %u\ti: %zu\tl: %zu\tstack_n: %d\n", k, i, limit, stack_n);)
    _P_TEXT_HIGHLIGHTED();
    if (tokens[k].kind == REMIMU_KIND_CARET) {
      ...
      if (i == 0 && !_REGEX_CHECK_IS_W(text[i]))
        ...
        _REWIND_OR_ABORT();
               ...
               _REGEX_CHECK_IS_W(text[i - 1]) == _REGEX_CHECK_IS_W(text[i]))
        _REWIND_OR_ABORT();
      ...
      if (tokens[k].count_hi == 1) {
        if (tokens[k].kind == REMIMU_KIND_OPEN ||
            tokens[k].kind == REMIMU_KIND_NCOPEN)
      ...
      if (tokens[k].kind == REMIMU_KIND_OPEN ||
        ...
        if (!just_rewinded) {
          ...
          // if we're lazy and the min length is 0, we need to try the non-group
          // case first
          if ((tokens[k].mode & REMIMU_MODE_LAZY) &&
              (tokens[k].count_lo == 0 ||
               q_group_accepts_zero[tokens[k + tokens[k].pair_offset]
                                        .mask[0]])) {
            IF_VERBOSE(puts("trying non-group case first.....");)
            range_min = 0;
            range_max = 0;
            _REWIND_DO_SAVE(k);
            k += tokens[k].pair_offset; // automatic += 1 will put us past the
                                        // matching )
          } else {
            range_min = 1;
            range_max = 0;
          ...
          IF_VERBOSE(printf("--- trying to try another alternation, start k is "
                            "%d, rmin is %zu\n",
                            k, range_min);)
          ...
          if (range_min != 0) {
            ...
            if (tokens[k].kind == REMIMU_KIND_CLOSE) {
              ...
              if (tokens[k].count_lo == 0 ||
                  q_group_accepts_zero[tokens[k].mask[0]]) {
          ...
          IF_VERBOSE(
              printf("--- FOUND ALTERNATION for paren at k %zd at k %d\n",
                     orig_k, k);)

          ptrdiff_t k_diff = k - orig_k;
          range_min = k_diff + 1;

          IF_VERBOSE(puts("(saving in paren after rewinding and looking for "
                          "next regex token to check)");)
        ...
        // quantified
          ...
          if (!just_rewinded) {
            ...
            IF_VERBOSE(
                printf(
                    "qrqrqrqrqrqrqrq-------      k %d, gs %d, gaz %d, i %zd, "
                    "tklo %d, rmin %zd, tkhi %d, rmax %zd, prev %d, sn %d\n",
                    k, q_group_state[tokens[k].mask[0]],
                    q_group_accepts_zero[tokens[k].mask[0]], i,
                    tokens[k].count_lo, range_min, tokens[k].count_hi,
                    range_max, prev, stack_n);)

            range_max = tokens[k].count_hi;
            range_max -= 1;
            range_min = q_group_accepts_zero[tokens[k].mask[0]]
                            ? 0
                            : tokens[k].count_lo;
            // REMIMU_ASSERT(q_group_state[tokens[k +
            // tokens[k].pair_offset].mask[0]] <= i); if (prev)
            // REMIMU_ASSERT(rewind_stack[prev].i <= i);
            IF_VERBOSE(
                printf("qzqzqzqzqzqzqzq-------      rmin %zd, rmax %zd\n",
                       range_min, range_max);)

            // minimum requirement not yet met
            if (q_group_state[tokens[k].mask[0]] + 1 < range_min) {
              IF_VERBOSE(
                  puts("continuing minimum matches for a quantified group");)
              q_group_state[tokens[k].mask[0]] += 1;
              _REWIND_DO_SAVE(k);

              k += tokens[k].pair_offset; // back to start of group
              k -= 1; // ensure we actually hit the group node next and not the
                      // node after it
              continue;
            }
            // maximum allowance exceeded
            else if (tokens[k].count_hi != 0 &&
                     q_group_state[tokens[k].mask[0]] + 1 > range_max) {
              IF_VERBOSE(printf("hit maximum allowed instances of a quantified "
                                "group %d %zd\n",
            ...
            // fallback case to detect zero-length matches when we backtracked
            // into the inside of this group after an attempted parse of a
            ...
            if ((force_zero ||
                 (prev != 0 &&
                  rewind_stack[prev].i ==
                      i))) //  && q_group_state[tokens[k].mask[0]] > 0
            {
            ...
            } else if (tokens[k].mode & REMIMU_MODE_LAZY) // lazy
            {
              ...
              if (prev)
                IF_VERBOSE(printf("lazy doesn't think it's zero-length. prev i "
                                  "%zd vs i %zd (depth %d)\n",
            ...
            } else // greedy
            {
              ...
              if ((uint32_t)
                      q_group_state[tokens[k + tokens[k].pair_offset].mask[0]] <
                  (uint32_t)i) {
                IF_VERBOSE(puts("REWINDING FROM GREEDY NON-REWIND CLOSER");)
                q_group_state[tokens[k].mask[0]] += 1;
                _REWIND_DO_SAVE(k);
                k += tokens[k].pair_offset; // back to start of group
                k -= 1; // ensure we actually hit the group node next and not
                        // the node after it
              } else
            ...
            if (tokens[k].mode & REMIMU_MODE_LAZY) {
              ...
              // greedy. if we're going to go outside the acceptable range,
              // rewind
              IF_VERBOSE(
                  printf("kufukufu %d %zd\n", tokens[k].count_lo, range_min);)
              // uint64_t old_i = i;
              if (q_group_state[tokens[k].mask[0]] < range_min &&
                  !q_group_accepts_zero[tokens[k].mask[0]]) {
                IF_VERBOSE(printf("rewinding from greedy group because we're "
                                  "going to go out of range (%d vs %zd)\n",
        ...
        if (!just_rewinded) {
          ...
          uint64_t old_i = i;
          while (n < tokens[k].count_lo && text[i] != 0 &&
                 _REGEX_CHECK_MASK(k, text[i])) {
            i += 1;
            n += 1;
          }
          ...
          if (tokens[k].mode & REMIMU_MODE_LAZY) {
            ...
            if (ilimit == 0)
              ilimit = ~ilimit;
            range_min = n;
            while (text[i] != 0 && _REGEX_CHECK_MASK(k, text[i]) &&
                   n + 1 < ilimit) {
            ...
            range_max = n;
            IF_VERBOSE(printf("set rmin to %zd and rmax to %zd on entry into "
                              "normal greedy token with k %d\n",
          ...
          if (tokens[k].mode & REMIMU_MODE_LAZY) {
            ...
            if (_REGEX_CHECK_MASK(k, text[i]) && text[i] != 0 &&
                range_min < ilimit) {
            ...
            // IF_VERBOSE(printf("comparing rmin %zd and rmax %zd token with k
            // %d\n", range_min, range_max, k);)
...
REMIMU_FUNC_VISIBILITY void print_regex_tokens(RegexToken *tokens) {
  ...
  for (int k = 0;; k++) {
    printf("%s\t%s\t", kind_to_str[tokens[k].kind],
           mode_to_str[tokens[k].mode]);
...
#define _PRINT_C_SMART(c)                                                      \
  {                                                                            \
    ...
    */
    ...
    printf("\t{%d,%d}\t(%d)\n", tokens[k].count_lo, tokens[k].count_hi - 1,
