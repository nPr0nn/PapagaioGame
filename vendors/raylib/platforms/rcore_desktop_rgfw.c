 ...
 *   LICENSE: zlib/libpng
 *
 *   Copyright (c) 2013-2025 Ramon Santamaria (@raysan5), Colleague Riley and
 *contributors
 *
...
void ToggleFullscreen(void) {
  if (!CORE.Window.fullscreen) {
    ...
    RGFW_window_move(platform.window, RGFW_POINT(CORE.Window.position.x,
                                                 CORE.Window.position.y));
    RGFW_window_resize(platform.window,
                       RGFW_AREA(CORE.Window.previousScreen.width,
...
void MaximizeWindow(void) { RGFW_window_maximize(platform.window); }

// Set window state: minimized
void MinimizeWindow(void) { RGFW_window_minimize(platform.window); }
...
void ClearWindowState(unsigned int flags) {
  ...
  if (flags & FLAG_WINDOW_RESIZABLE) {
    RGFW_window_setMaxSize(
        platform.window, RGFW_AREA(platform.window->r.w, platform.window->r.h));
  ...
  if (flags & FLAG_WINDOW_UNFOCUSED) {
    RGFW_window_setFlags(platform.window,
                         platform.window->_flags | RGFW_windowFocusOnShow);
    CORE.Window.flags &= ~FLAG_WINDOW_UNFOCUSED;
  }
  if (flags & FLAG_WINDOW_TOPMOST) {
...
int RGFW_formatToChannels(int format) {
  ...
  case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16: // 16*4 bpp (4 channels - half
                                              // float)
...
void SetWindowIcon(Image image) {
  RGFW_window_setIcon(platform.window, image.data,
                      RGFW_AREA(image.width, image.height),
                      RGFW_formatToChannels(image.format));
}
...
void SetWindowIcons(Image *images, int count) {
  if ((images == NULL) || (count <= 0)) {
    ...
    for (int i = 0; i < count; i++) {
      if ((bigIcon == NULL) || ((images[i].width > bigIcon->width) &&
                                (images[i].height > bigIcon->height)))
        bigIcon = &images[i];
      if ((smallIcon == NULL) || ((images[i].width < smallIcon->width) &&
                                  (images[i].height > smallIcon->height)))
        smallIcon = &images[i];
    }

    if (smallIcon != NULL)
      RGFW_window_setIconEx(platform.window, smallIcon->data,
                            RGFW_AREA(smallIcon->width, smallIcon->height),
                            RGFW_formatToChannels(smallIcon->format),
                            RGFW_iconWindow);
    if (bigIcon != NULL)
      RGFW_window_setIconEx(platform.window, bigIcon->data,
                            RGFW_AREA(bigIcon->width, bigIcon->height),
                            RGFW_formatToChannels(bigIcon->format),
                            RGFW_iconTaskbar);
  }
}
...
// Set window focused
void SetWindowFocused(void) { RGFW_window_focus(platform.window); }
...
#define MAX_MONITORS_SUPPORTED 6
  ...
  for (int i = 0; i < 6; i++) {
    if (!mons[i].x && !mons[i].y && !mons[i].mode.area.w &&
        mons[i].mode.area.h) {
      count = i;
      break;
    }
...
int GetCurrentMonitor(void) {
  ...
  if (platform.window)
    mon = RGFW_window_getMonitor(platform.window);
  else
    mon = RGFW_getPrimaryMonitor();

  for (int i = 0; i < 6; i++) {
    if ((mons[i].x == mon.x) && (mons[i].y == mon.y))
      return i;
  }
...
Vector2 GetWindowScaleDPI(void) {
  ...
  if (platform.window)
    monitor = RGFW_window_getMonitor(platform.window);
  else
...
const char *GetClipboardText(void) { return RGFW_readClipboard(NULL); }
...
#else
  TRACELOG(LOG_WARNING, "Clipboard image: PLATFORM_DESKTOP_RGFW doesn't "
                        "implement GetClipboardImage() for this OS");
#endif
...
void SwapScreenBuffer(void) { RGFW_window_swapBuffers(platform.window); }
...
double GetTime(void) { return RGFW_getTime(); }
...
void PollInputEvents(void) {
  ...
  if ((CORE.Window.eventWaiting) || (IsWindowState(FLAG_WINDOW_MINIMIZED) &&
    ...
    RGFW_window_eventWait(platform.window,
                          -1); // Wait for input events: keyboard/mouse/window
                               // events (callbacks) -> Update keys state
  ...
  while (RGFW_window_checkEvent(platform.window)) {
    ...
    case RGFW_DND: // Dropped file
    {
      for (int i = 0; i < event->droppedFilesCount; i++) {
        if (CORE.Window.dropFileCount == 0) {
          ...
          // all possible dropped files at the moment we limit the number of
          // drops at once to 1024 files but this behaviour should probably be
          // reviewed
    ...
    case RGFW_windowResized: {
      ...
      // if we are doing automatic DPI scaling, then the "screen" size is
      // divided by the window scale
      if (IsWindowState(FLAG_WINDOW_HIGHDPI)) {
        CORE.Window.screen.width =
            (int)(platform.window->r.w / GetWindowScaleDPI().x);
        CORE.Window.screen.height =
            (int)(platform.window->r.h / GetWindowScaleDPI().y);
      } else {
    ...
    case RGFW_keyPressed: {
      ...
      // codepoints (int) Check if there is space available in the queue
    ...
    case RGFW_mouseButtonPressed: {
      ...
      if (btn == RGFW_mouseLeft)
        btn = 1;
      else if (btn == RGFW_mouseRight)
        btn = 2;
      else if (btn == RGFW_mouseMiddle)
        btn = 3;
    ...
    case RGFW_gamepadConnected: {
      ...
      CORE.Input.Gamepad.axisCount[platform.window->event.gamepad] =
          platform.window->event.axisesCount;
      CORE.Input.Gamepad.axisState[platform.window->event.gamepad]
                                  [GAMEPAD_AXIS_LEFT_TRIGGER] = -1.0f;
      CORE.Input.Gamepad.axisState[platform.window->event.gamepad]
                                  [GAMEPAD_AXIS_RIGHT_TRIGGER] = -1.0f;

      strcpy(
          CORE.Input.Gamepad.name[platform.window->event.gamepad],
          RGFW_getGamepadName(platform.window, platform.window->event.gamepad));
    } break;
    ...
    case RGFW_gamepadAxisMove: {
      ...
      case 0: {
        CORE.Input.Gamepad.axisState[event->gamepad][GAMEPAD_AXIS_LEFT_X] =
            event->axis[0].x / 100.0f;
        CORE.Input.Gamepad.axisState[event->gamepad][GAMEPAD_AXIS_LEFT_Y] =
            event->axis[0].y / 100.0f;
      } break;
      case 1: {
        CORE.Input.Gamepad.axisState[event->gamepad][GAMEPAD_AXIS_RIGHT_X] =
            event->axis[1].x / 100.0f;
        CORE.Input.Gamepad.axisState[event->gamepad][GAMEPAD_AXIS_RIGHT_Y] =
            event->axis[1].y / 100.0f;
      } break;
      case 2:
...
int InitPlatform(void) {
  ...
  if (!(CORE.Window.flags & FLAG_WINDOW_UNFOCUSED))
    flags |= RGFW_windowFocusOnShow | RGFW_windowFocus;

  platform.window = RGFW_createWindow(
      CORE.Window.title,
      RGFW_RECT(0, 0, CORE.Window.screen.width, CORE.Window.screen.height),
      flags);
  platform.mon.mode.area.w = 0;
  ...
  if (RGFW_useWaylandBool)
    TRACELOG(LOG_INFO,
             "PLATFORM: DESKTOP (RGFW - Wayland): Initialized successfully");
  else
    TRACELOG(
        LOG_INFO,
        "PLATFORM: DESKTOP (RGFW - X11 (fallback)): Initialized successfully");
#elif defined(RGFW_X11)
#if defined(__APPLE__)
  TRACELOG(LOG_INFO,
           "PLATFORM: DESKTOP (RGFW - X11 (MacOS)): Initialized successfully");
#else
  TRACELOG(LOG_INFO,
           "PLATFORM: DESKTOP (RGFW - X11): Initialized successfully");
#endif
#elif defined(RGFW_WINDOWS)
  TRACELOG(LOG_INFO,
           "PLATFORM: DESKTOP (RGFW - Win32): Initialized successfully");
#elif defined(RGFW_WASM)
  TRACELOG(LOG_INFO,
           "PLATFORM: DESKTOP (RGFW - WASMs): Initialized successfully");
#elif defined(RGFW_MACOS)
  TRACELOG(LOG_INFO,
           "PLATFORM: DESKTOP (RGFW - MacOS): Initialized successfully");
#endif
...
// Close platform
void ClosePlatform(void) { RGFW_window_close(platform.window); }

// Keycode mapping
static KeyboardKey ConvertScancodeToKey(u32 keycode) {
  if (keycode > sizeof(keyMappingRGFW) / sizeof(unsigned short))
    return 0;
